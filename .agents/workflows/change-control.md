...
2. Si es **CREATE**, informar el impacto y esperar confirmación del usuario para generar el documento en `docs/changes/`.
3. Si es **APPROVE**, ejecutar cambios, añadir trazabilidad y actualizar tokens de gobernanza si aplica.

## Paso 2 — Verificación de Trazabilidad
Asegurar que el archivo en `docs/changes/` refleje el estado final (ID, Fecha, Decisión) y que se cumpla la jerarquía de `CLAUDE.md`.
